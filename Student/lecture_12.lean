...
| or
| imp
| iff -- added for HOMEWORK QUESTION #4
...
infixr:20 " ⇔ " => Expr.bin_exp binary_op.iff -- HOMEWORK QUESTION #4
...
def biimplication : Bool → Bool → Bool --HOMEWORK QUESTION #4
| true, true => true
| false, false => true
| _, _ => false -- either of the two other combos of T/F would be false

def eval_bin_op : binary_op → (Bool → Bool → Bool)
| binary_op.and => and
| binary_op.or => or
| binary_op.imp => implies
| binary_op.iff => biimplication --HOMEWORK QUESTION #4
...
| (Expr.bin_exp op e1 e2), i => (eval_bin_op op) (eval_expr e1 i) (eval_expr e2 i) -- applying bin_op.iff to interpretation and expression would return Bool based on definition of biimplication
...
#eval c.n
#eval var.n c -- test: equivalent to the prev line
...
def B := {b}  --we want B to correspond to the expression form of our var b. So "b" is a var while "B" is now a var expr
-- ^ is an atomic propositional *expression*
def C := {c}
def J := {j}
def A := {a}
...
-/
 ...
 --*Associated code added between lines 28 and 66*
...
#eval eval_expr e0 (λ _ => false) -- expect true. Lambda expression means whatever var expr passed in, will be false. Is it true that if both var exprs J and C evaluate to false, then the overall expr will be true? Yes. Lambda expression takes place of the interpretation
#eval eval_expr e0 (λ _ => true)  -- expect true

-- You do the rest
#eval eval_expr e1 (λ _ => false) -- expect true because if A and B are both false, ((not false) or (not false)) simplifies to *(true or true)* simplifies to *true* on the left of the =>. Then, we have *not(false and false)*, which simplifies to *not(false)* simplifies to *true*. True can indeed imply true, so the expression overall evaluates to true.
...
def some_true: Interp
| (var.mk 0) => true -- lowercase j = true
| (var.mk 1) => false
| (var.mk 2) => true
| (var.mk 3) => false
| _ => false 
...
#eval eval_expr e0 some_true --expect true
#eval eval_expr e1 some_true --expect true
#eval eval_expr e2 some_true --expect true
#eval eval_expr e3 some_true --expect true
