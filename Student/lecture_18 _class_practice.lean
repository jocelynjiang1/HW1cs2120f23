...
/-!
## Homework
--predicate: a parameterized proposition, like a statement with a blank in it.
(1) Define a predicate, ev_len_str, expressing the property
of a string of being of an even-length.
-/

-- Here
def ev_len_str: String → Prop -- it'll be a parameterized proposition, aka a function that takes some values and returns a proposition with those values put into right places
| s => s.length %2 = 0

#reduce ev_len_str "hi"
#reduce ev_len_str "hello"
...
-- Here
def set_of_evens : Set (String) := {s | s.length%2 = 0} --set/display notation: notation with the curly braces
#check set_of_evens
--or HIS ANSWER:
#check {s: String | ev_len_str s}-- set notation here means the set of any String s, such that ("|") the application of that predicate to s
...
-- Here
def str_eq_len : String → Nat → Prop -- rememebr: predicated = parameterized proposition = a function returning a proposition
| s, n => s.length = n
#reduce str_eq_len "hi" 2
...
-- if you hover over p, Lean knows p is a pair of String and Nat. p.1 is the left side of the pair; p.2 is right side
...
-- we are asserting the pair "I love logic" and 13 are in the set str_eq_lens
--apply the predicate to the object to get a proposition
...
-/
--symbol \ nin gives ∉, is not in
-- Here
example: ⟨ "I love Logic!", 1 ⟩ ∉ str_eq_lens := -- a proof of this will be a proof of a function
λ (t: ⟨ "I love Logic!", 1 ⟩ ∈ str_eq_lens) => nomatch t --nomatch generates empty matches
--a function (lambda) that takes a proof that the object IS in the predicate and returns false
