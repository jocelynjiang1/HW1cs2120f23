...
Answer here: String → Nat function
-/
...
#eval glue_funs' isEven String.length "Hello"  -- false /-because String length is 5, which isn't even.-/
#eval glue_funs' isEven String.length "Hello!" -- true /-because String length is 6-/
#eval glue_funs' isEven String.length "Here's a sentence to test with" -- true, 30 characters
/-!
...
def glue_funs : {β γ α : Type} →  (β → γ) → (α → β)→ α → γ 
| _, _, _,g, f, a => g (f a)
...
#eval glue_funs double square 5 -- expected: 50 because squares 5 first
#eval glue_funs square double 5 -- expected: 100 because doubles 5 first
