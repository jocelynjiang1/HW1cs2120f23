...
def prod_comm { α β : Type } : α × β → β × α -- this line: a "logical proposition"
| (g, f) => (f, g) -- writing an executable proof that the type signature ^ works

-- testing
#eval prod_comm (1,5) -- expect (5,1)
...
| Prod.mk g f => Prod.mk f g

-- testing
#eval prod_comm (5, 1) -- expect (1,5)
...
-- testing
#eval prod_assoc (1, (3, 5)) -- expect ((1, 3),5)
...
-- testing
#eval prod_assoc_reverse ((1, 3), 5) -- expect (1,3,5), which is the same as (1, (3, 5))
...
/- testing with prod as an example
def prod {α β : Type}: α × β → β × α :=
fun s =>
  match s with
...
#eval sum_comm a_sum1
#eval sum_comm b_sum1 -- expect Sum.inl true (because was defined in b_sum1 as second element but should be able to output as first element)
/-!
...
--testing
def a_sum2 : Sum Bool Nat := Sum.inl true
def b_sum2 : Sum Bool Nat := Sum.inr 1
#eval sum_comm_reverse a_sum2 -- expect Sum.inr true since Bool given as first element (left side of metaphorical two-compartment box) and should be able to alternatively be second element
#eval sum_comm_reverse b_sum2
...
| (Sum.inl a) => (Sum.inl (Sum.inl a)) -- "case analysis"
| (Sum.inr (Sum.inl b)) => (Sum.inl (Sum.inr b))
| (Sum.inr (Sum.inr g)) => (Sum.inr g)

-- testing
def a_sum3 : Sum Nat (Sum Bool Nat) := Sum.inr (Sum.inl false)
#eval sum_assoc a_sum3 -- expect Sum.inl (Sum.inr false), which would mean *false* was able to be associated with α and not just γ
...
-- testing
def a_sum4 : (Sum (Sum Nat Bool) Nat) := Sum.inl (Sum.inr true)
#eval sum_assoc_reverse a_sum4 -- expect Sum.inr (Sum.inl true)
...
a *γ* value. 
 ...
 | Prod.mk a (Sum.inr g) => Sum.inr (Prod.mk a g)

-- testing
def a_sum5 : Sum Nat Bool := Sum.inl 5
def a_sum6 : Sum Nat Bool := Sum.inr true
#eval prod_dist_sum ("Hi", a_sum5)
#eval prod_dist_sum ("Hi", a_sum6)
...
-- Here:
 ...
 | Sum.inl (Prod.mk a b) => Prod.mk a (Sum.inl b)
 | Sum.inr (Prod.mk a g) => Prod.mk a (Sum.inr g)
...
-- Here
def its_wet: (sprinkler → wet) → (rain → wet) → (rain ⊕ sprinkler) → wet
| _, g, (Sum.inl a) => (g a) -- clicking here show return type would be wet
| f, _, (Sum.inr a )=> (f a) -- clicking here show return type would be wet
...
-- Here:
-- where β = sprinkler; α = rain; γ = wet
def sum_elim {α β γ : Type}: (β → γ) → (α → γ) → (α ⊕ β) → γ 
| _, g, (Sum.inl a) => (g a) -- clicking next to either line shows return type would correctly be γ 
| f, _, (Sum.inr a) => (f a)
