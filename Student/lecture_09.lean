...
-/
--Unit type in Python is "none", in Java it's "void"
--Empty type can return anything, so useless in programming but useful logically because corresponds to *false*.
namespace cs2120
...
inductive Empty : Type -- no constructors, has no values because no way to construct any if no constructors. (Only way to build one of the types, whether sum product or unit or empty, is with a constructor, except empty has none)
...
-- Empty is uninhabited type. Type w/ at least one value is an inhabited type.
...
def inhabited_to_empty : Unit → Empty --function is also uninhabited because there's no way to finish defining or implement this func
| unit => _  -- can't write a term of the Empty type     
--NOTE: this pattern match basically says True => False, which is FALSE (false proposition). Think of implies case
/-!
...
def empty_value := empty_to_empty' _ -- no way to apply it/can't be called
...
def empty_to_bool :         Empty → Bool := nomatch e -- ~ "False implies true" = true
def empty_to_nat :          Empty → Nat  := nomatch e  -- inhabited
def empty_to_α (α : Type) : Empty → α    := nomatch e -- inhabited
--you can always write a func of type empty to something else; it's F > T which is T. We can never apply it, though.
/-!
...
def empty_elim (α : Type) : Empty → α := nomatch e -- means α must itself be uninhabited (Empty). Only False -> False is True. False -> True is False, so given this func works, only possible e is False, or empty.
...
- Can you define some function, nxe2s : Nat × Empty → String No
- Is the type, Nat × Empty → String, inhabited or not?
- How many strings can nxe2s possibly return? Why?
- Can you define a function, noe2s : String ⊕ Empty → Nat Yes (you could do String.length--question just means could String be an arg and be turned into a Nat)
- Is the type, String ⊕ Empty → Nat, inhabited or not?
- Can noe2s return any Nat? If so, prove it by example.
- Is the function type, (Nat → Empty), inhabited or not? Uninhabited (false)
- Prove your answer (is (Nat → Empty) uninhabited)
- Is the type, {α : Type} → α → Empty, inhabited or not. Uninhabited (can't return a result of type empty)
- Prove your answer (Is {α : Type} → α → Empty, inhabited?)
- Is the type, Empty → (Nat → Empty) inhabited? Prove it. From false, anything follows. so if given a false, you can return anything.
- Prove this type uninhabited: {α : Type} → α × (α → Empty)
-/

-- Example: def f2e (f: Foo) : Empty := nomatch f
-- wher enomatch f means no val for f, aka if f is uninhabited, makes this work.
