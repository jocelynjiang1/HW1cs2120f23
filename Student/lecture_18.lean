...
/-!
## Predicates
--proposition = declaritive statement asserting some state of affairs is true
--
...
/-!
### Families of Propositions -- we will use syntactic rather than semantic approach
--an infinite family of proposiitons

Indeed, there is an infinite family of propositions, all just --propositions of parameters...
...
We can write this family of propositions by *abstracting* the value, --a predicate really is a funciton from values to propositions
...
def is_even : Nat → Prop := λ n => n % 2 = 0 --n %2 is a proposition, an assertion that theis equation holds
--predicate:just some func from some domain to another place.
#reduce is_even 4 -- evaluate to 0 is equal to 0.is_even--is_even is a predicate.  Consumes some arguments,  yields a proposition
...
-/
#check (is_even)      -- Nat → Prop --what's the type of this? Nat → Prop
--predicate is just some func that takes values and plugs them in

/-!
### Applying a Predicate to Arguments Yields a Proposition -- proofs demonstrate validity of a proposition
...
--4, 2, 6, 8, all other even numbers satisfy the is_even predicate
--predicate: picking out values that satisfy property (here, being even, which is captured by n%2 = 0)
...
#reduce is_even 0   -- ✓
#reduce is_even 1   -- ×
#reduce is_even 2   -- ✓ --all even numbers reduce to 0 = 0. is_even gives rise to family of props
#reduce is_even 3   -- ×
#reduce is_even 4   -- ✓
#reduce is_even 5   -- ×
-- a set is colleciton of _ that satisfies some predicate. We'll learn sets based on predicates
/-!
...
def evens : Set Nat := { n | is_even n } --this specifies set of even numbers.
--predicate yields a result that'as either true or false
--predicate can be multiple args(?)
#reduce evens 4
#reduce evens 5
...
-/
--membership in a set is determined by satisfaction of some complex condition
def square_pair : Nat × Nat → Prop --takes one arg, but that arg is an ordered pair
| (n1, n2) => n2 = n1^2
...
def square_pairs : Set (Nat × Nat) := { p : Nat × Nat | square_pair (p.1, p.2) } --even more specifically defined than prev
--I want square_pair to be pair that satisfies the square_pair predicate
-- square_pairs is a set of all the objects that satisfy the condition. Pairs that don't satsify square_pair aren't part of the set.
--If osmething is in set, then resulting predicate is true.
#reduce square_pairs
#reduce (3, 9) ∈ square_pairs --symbol means "is a member of _", aka (3,9) satisfies square_pairs

/-!
#### Pythagorean triples
-/
--how do we specify pythag triples?
--pythagorean_triple is a predicate (and aka a function)
def pythagorean_triple : Nat → Nat → Nat → Prop
| h, x, y => h^2 = x^2 + y^2

#reduce pythagorean_triple 5 3 4 --order here matters since h is to represent the hypotenuse

def py_trips: Set (Nat x Nat x Nat) := {t | t.1^2 = t.2.1^2 + t.2.2^2} --t.1 is the hypotenuse
...
--There's a predicate that's true for all...
#reduce ev_len "Hello!"


def ev_len_strs: Set String:= {s| is_even (s.length)}
--set membership is a logical, not computation set. Not like hash set in Java
--set intersection = value(s) that two sets share.

def up_to_three: Set Nat := {n | n = 1 ∨ n = 2} --this is set containing 1 and 2
#reduce 1 ∈ up_to_three -- yes, it's in set because it satisfies 1 = 1 case returned (return is some Or statement)
--get ∈ by typing "\in"
/-!
## Quantifiers
...
- ∀ (x : T), P x --for any object of type x has proposition P.
- ∃ (x : T), P x
--Example: every dog is friendly. Could write as for all d of type Dog, Friendly will be satisfied
-- ∀ (d: Dog), Friendly d
--∀ (p q: Person), Loves p q --means everyone loves everyone (every person Loves q, which is also people)
-- ∀ (p: P), there is (backwards E symbol) (q: Person), Loves p q
