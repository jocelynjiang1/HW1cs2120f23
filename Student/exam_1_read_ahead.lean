...
| not --this does not refer to Lean's defined Boolean not
...
inductive Expr : Type -- defines meaning of things of type Expr--what is an Expr? 3 types
| var_exp (v : var) --an Expr might take just a var. In that case, it'll be a var_exp
| un_exp (op : unary_op) (e : Expr)
| bin_exp (op : binary_op) (e1 e2 : Expr) -- might take a binary_op and two types of Expr. This overall Expr would be a bin_exp (binary expression)
...
/-!
### Semantics --giving meanings to all Types whose structure is laid out above^
-/

-- *meanings* of unary operators
def eval_un_op : unary_op → (Bool → Bool) --takes a unary_op and returns that unary_op's meaning
| unary_op.not => not -- not on right side of "=>" refers to Lean's defined Boolean not.
...
| binary_op.and => and --already defined in Lean
| binary_op.or => or --already defined in Lean
| binary_op.imp => implies --defined above
| binary_op.iff => iff --defined above
...
| (Expr.var_exp v),        i => i v --if given a {variable} and an interpretation, just returns interp applied to {var} (aka the var's Bool val)
| (Expr.un_exp op e),      i => (eval_un_op op) (eval_expr e i) -- uses func that assigned meaning to unary op, aka uses eval_un_op which always evaluates to the Lean-defined Bool op not.
...
| 0     => [0]
| 1     => [1]
| n' + 2 => -- just n'+2 meaning Nat passed for this option must be >=2
...
| r, c => zero_pad c (nat_to_bin r) -- c gives how many 0s to put, nat_to_bin r gives the List Nat rep'ing row# in bits to add those 0s in front of
