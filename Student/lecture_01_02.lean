...
will tell you that each of these is a Type. #check will tell you what type/kind of term the term is. Some terms are of the type Type.
-/
...
#check (true && false)    -- using "infix" notation for "and" (same as previous line (?))
...
-- Takes two Booleans and returns a Boolean / Being right-associative means you look at right-most term, and that's assumed as the result. What precedes arrow to this term assumed the input(s). However, if parentheses in place, output or input terms might change.
...
#eval String.append s1 s2 --functions when applied most often use prefix notation in Lean
...
-/
def s3 := String.append s1 s2 --def is used for creating variables, functions, and function applications
#eval s3
...
#eval (String.append "Hello, ") "Lean!" /-what's inside () returns function that will append next item to the first, "Hello," REMEMBER that function applications are left-associative, which is why here (a function application), parentheses around left side is same as without any-/
...
/-
Questions:
- Which two types are equivalent? #2 & #3
- Are #1 and #3 equivalent? no
...
#check (not) /-and, not, or are functions-/
#check (and)
#check (or)
...
| true, true => false /-my answers to the self-test-/
| true, false => false
| false, true => false
| false, false => true
#eval nor true true
...
#eval mystery false false
#eval mystery true true /- these from preceding results match the AND of both negated args-/
