...
#reduce no Nat -- test: no Nat is Nat → Empty, so (below) no jam is jam → Empty, meaning a func that takes jam and returns empty. So nojam j gives Empty
/-!
...
def not_either_not_both { jam cheese } :
  ((no jam) ⊕ (no cheese)) → -- existence of a function jam → empty means jam can have no value in this func.
  (no (jam × cheese)) -- if neither jam nor cheese, then there isn't both
| Sum.inl nojam => (fun (j, c) => (nojam j)) -- If we are given nojam, jam and cheese returns empty. Can use fun keyword or λ--same thing. 
| Sum.inr nocheese => (fun (j,c) => (nocheese c))
--In line 79 we have a contradiction: assuming nojam with "Sum.inl nojam", then asserting we have jam with "fun (j,c)". A contradiction, which returns false (empty?)
--note in 79 that *nojam* is a function, taking a value of type jam and returning empty (because nojam matches to no jam, which is a func jam → Empty)
--arguments to functionso are assumptions
--*Be able to do these*. Put underscores and figure out implementation based on what you already established and still need/where you're going
...
def demorgan1  {α β : Type} : ((α → Empty) ⊕ (β → Empty)) → (α × β → Empty)  --DeMorgan: not (x and y) = not x or not y //*one of them must be false given both is not true*
...
def demorgan2 {α β : Type} : (α ⊕ β → Empty) → ((α → Empty) × (β → Empty)) -- first arg same as no(α *circle plus* β ), as in I don't have either α or β
| noaorb => Prod.mk (fun a => noaorb (Sum.inl a)) (fun b => noaorb (Sum.inr b))
--"Assume the premise, show the conclusion"
  --noaorb is a func taking sum α β, returns a val of type empty.
  --so noaorb ("not either") returns a product type of two functions that return empty (not one AND not the other)
  --α → Empty, first part of returned pair, is not empty. It is a function that proves there can be no α
...
-/
def demorgan3 {α β : Type} : ((α → Empty) × (β → Empty)) → ((α ⊕ β) → Empty) -- remember *____ implies empty* basically means *no ____*  
| (noa, nob) => 
  fun s =>
  match s with --because working with sum type, do case analysis
  | Sum.inl a => noa a
  | Sum.inr b => nob b
...
def n5 := Nat.succ n4
#eval n0
#eval n1 -- Nat.succ outputs its argument + 1
...
-- Test cases
#reduce pred 3    -- expect 2; basically 3 matches to "Nat.succ n." Makes it so n is enveloped in the Nat.succ n, so to get n back, must "undo" a Nat.succ done to 3. Get 2 back
...
-/
inductive Doll: Type -- Based on Lecture 10
| solid
| shell (little_doll: Doll)

open Doll

def unravel: Doll → Doll
| solid => solid
| shell (little_doll) => little_doll
...
| 0, 0 => true
| 0, n' + 1 => false -- n' + 1 is a way of representing values greater than 0. If second Nat passed in is 5, n' + 1 is 5, and n' would be 4
| n' + 1, 0 => false
| (n' + 1), (m' + 1) => nat_eq n' m' --say given (n' + 1 = 5), (m' + 1 = 3). If we return nat_eq n' m', will take 4 and 2 and plug into func again. Func will direct to this step where it will calc nat_eq 3 1, then 2 0 when finally arrive at third case, where they are false.
...
| Nat.zero, Nat.succ n' => true
| (Nat.succ n'), Nat.zero => false
| Nat.succ n', Nat.succ m' => nat_le n' m'

#reduce nat_le 1 5 -- correctly returns true
#reduce nat_le 5 2 -- correclty returns false
...
def add : Nat → Nat → Nat
| m, 0 => m
| m, (Nat.succ n') => add (Nat.succ m) n'  --As n' gets smaller each time this line is executed until n' is 0, m grows at the same time until it is essentially add (m + n') 0, which will return the first arg (m +) 


-- Some test cases
#reduce add 0 0   -- expect 0 --all work
...
def mul : Nat → Nat → Nat
| m, 0 => 0
| m, (Nat.succ n') => add ((mul m n')) (m) -- right side using distributive property
...
| f, 0 => 0
| f, n' + 1 => (sum_f f n') + (f (n' + 1)) -- ***modified since version submitted. Basically summing everything from 0 to f (n'), then adding f (n'+1)
--| f, n' + 1 => Nat.add (f (n' + 1)) (sum_f f n') --His is same

#reduce sum_f Nat.succ 4
#reduce sum_f (λ n => n) 10
#reduce sum_f (λ n => n*n) 5
