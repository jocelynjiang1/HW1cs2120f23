...
*wffs* include the following: (as in the following are random, some of many examples of acceptable/meaningful and valid expressions in arithmetic)
- *1 + 113
- 10 = 11* 
- "x * y = 1" -- meaning of expression depends on interpretations of x and y. Many possible evaluations in this case
given an expression with variables in it and evaluations of those variables, 
...
expressions. 
- In the first example, we interpret *+* as arithmetic addition, and the overall arithmetic expression then *means* 114. 
...
Speaking informally, the semantics of arithmetic is *("semantics of arithmetic" as in the meaning/purpose of arithmetic, I think)
...
- It's raining -- add "is it true that" in front of the statement to test if it's a proposition. If sentence makes sense, then it's a proposition. Proposition is statement about state of affairs
- The ground is wet
- x * y = 1
...
- If it's raining then the ground is wet -- not atomic because it's made of two smaller propositions (one large proposition overall)
- a implies b (using propositional letters, so a and b are each a proposition)
...
propositional logic:
- {a}           -- it's raining --it's raining is a variable, curly braces turns it's raining into an expression.
- {b}           -- the ground is wet
- ¬a            -- it's not raining -- putting not in front makes it an even bigger proposition
- a ⇒ b         -- if it's raining then the ground is wet
- a ∨ b         -- it's raining or the ground is wet
- (a ∧ b) ∨ ¬a  -- (raining and wet) or (not raining) 

--NOTES:
- interpretation = func that takes a var and returns a value for that var
- turn atomic variable into expression by putting curly braces around
- if and only if = are equivalent = by conditional (a implies b and b implies a)
...
-/
inductive var' : Type
| mk (n : Nat) -- Using Lean prover's provided/established constructor mk?
...
#eval var.n v₂  -- application notation. According to his explanation with getters, seems it means n is the field name and the getter. Seems that would mean this line works like this: var.n <arg> = get that arg's return value from var. Since var(.mk) was applied to 2, seems the getter would return the value 2.
...
prefix:max "¬" => un_exp unary_op.not -- prefix notation: put symbol before the arg. ~ is an example
...
def e0 := {v₀} -- atomic expression from single variable
def e1 := ¬e0
def e2 := e0 ∧ e1
def e3 := e0 ∨ e1
def e4 := (e2 ∧ e3) ∨ e0
...
-/
def Interp := var → Bool  -- interp is a type. v0, v1, v2 etc are all of Type var.

-- examples
def all_true  : Interp := fun _ => true --interpretation where all variables are true
...
| unary_op.not => not -- I believe the "not" to the right of the arrow represents the recognized function(?) "not" in Lean
...
| var_exp v, i => i v
| un_exp op e, i => (eval_un_op op) (eval_expr e i) -- perform the right parenthesis set first, then having evaluated expr according to a given interpretation/valuation of the variables, evaluate an operator applied to that valuated expression
