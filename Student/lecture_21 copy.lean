...
A set is intuitively understood as a collection of objects. -- sets are collections of objects. They're predicates
...
def small := λ n : Nat => n = 0 ∨ n = 1 ∨ n = 2 ∨ n = 3 ∨ n = 4 -- small predicate specifies set of all numbers less than 5.
...
#reduce small 1
example: small 1 := Or.inr (Or.inl rfl) -- first layer of Or.inl would be 0 = 1 V (every thing else forms the Or.inr). The 1 = 1 would become Or.inl of that remaining Or.inr
...
#reduce s1   -- sets are predicates in Lean -- look at value: it is the predicate we saw earlier! Lean turns the set into a predicate
...
-- set of Natural numbers n such that n is 0 or n is 1 or...
...
-- set is a collection of objects. Formally, a ...
-- and two notations
...
### Membership (membership operation -- by applying set as a predicate to the object being checked for membership)
--item is in the set if it satisfies membership predicate
...
-- What does 1 is in small_set really mean? Just means *1 satisifes small_set predicate*
...
-/
#check 1 ∈ small_set
#reduce 1 ∈ small_set -- has same meaning as small_set applied to 1.
-- ∈ is \in
--Proof of 1 in small_set:  we can prove with small_set as a predicate, applied to 1
...
-/
#reduce small_set 3 --the logical proposition we need to prove to help us in following line:
...
#reduce univ        -- fun _a => True --univ is notation for universal set in Lean
#reduce univ 0      --           True -- no matter what arg you supply, get True back. As we can see, any natural number is a member of universal set.
#reduce univ 123456 --           True

/-!
#### Empty set

-- Set membership: 5 ∈ small_set, for example
-- Empty set membership: 5 ∈ ∅

The empty set of values of a given type denoted ∅ (phi, \empty) is the set
...
/-!
### Intersection
--This would be the overlap in a venn diagram
--underlying logic is like AND
...
-- s ∩ t's type is just another set! Also, ∩ is ∩
...
- What values are in the set, {1, 2, 3} ∩ {4, 5, 6}? none, so the resulting set is empty set.
...
example: 2 ∈ ev_set ∩ small_set := And.intro (rfl) (Or.inr (Or.inr (Or.inl rfl))) -- proof that 2 is in the intersection
-- we can give proof that 2 is in ev_set, and 2 is in small_set.
...
def even_or_small_set := ev_set ∪ small_set -- 0,1,2,3,4,6,8,10...small set constrained to 0-4, but evens can span beyond 4.
...
-/
--*Multiset: set that can contain duplicate elements*
#reduce 6 ∈ even_or_small_set -- was even_or_small_set defined incorrectly? Why is 6 = 6 not showing up as an option?
...
-- NOT 7 in ev_set, or 7 in even implies false. Assume 7 is in even, then show it derives a contradiction.

example : 7 ∈ ev_set := _ -- stuck, can't prove

/-!
### Set Complement -- third operation (unary operator)
...
#reduce Set.compl s   -- apply Set.compl operation without notation -- the predicate for the complement of s is what's in infoview
-- a ∈ s is the same as a ∉ s
...
-- [1,2,3] - [2,3,4] = [1]
#reduce Set.diff
example: 6 ∈ ev_set \ small_set := And.intro (rfl) (λ h => nomatch h) -- Hard to prove. First, check what proposition does this really mean?
#reduce 6 ∈ ev_set \ small_set -- an AND proposition. Now knowing the logical proposition, see that it's an AND we want to prove
--proving 6 is not in small_set: prove 6 in small_set returns false
...
-- Write down/figure out translations from set theory to underlying operations.
-- Set theory is a lang of its own. Be able to write propositions
