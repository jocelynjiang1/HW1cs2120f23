...
def ex_empty {α β : Type} : (α → Empty) → α → β -- false implies anything
| n, a => ((n a) → b) b--an empty to beta function applied to a beta would return a beta
...
-- Your answer here
inductive Sandwich (α β : Type) : Type
| mk (α : Bread) (β: Cheese ⊕ Spread)
/-!
...
def jam_sandwich : Sandwich Bread Spread := Sandwich.mk Bread.wheat (Sum.inr Spread.jam)
