...
inductive Doll : Type -- two possible Doll types. Remember inductive types are finitely constructed. We can have an infinite set of possible dolls since they can contain any number of layers, but each doll within is finitely constructed. This way we will eventually reach the solid inside.
| solid
| shell (d : Doll) -- shell with a doll inside, with that doll being a Doll, either also a shell or a solid
--building a doll: must start with solid because it's the one that takes no args
open Doll
...
def d1 := shell d0
def d2 := shell d1
def d3 := shell d2 -- same as shell of shell of shell of solid
...
-- inductive usually means it's a type built of smaller types. Imagine trees in programming: connect nodes and you get smaller parts building up a larger part, which could be the type we're defining
...
def inner: Doll → Doll --only two ways of building dolls: solid or shell with solid inside
| solid => solid -- func ftrying to get to the innermost doll
| shell (d')=> d' --it's shell holding a doll inside (d' is just a name, can be any name)

#reduce inner d3    -- expect (shell (shell solid)) -- because d3 is shell(shell(shell solid)), and when applying inner, we get shell(shell solid) as the innermost
...
def depth : Doll → Nat --func says if solid, depth is 0. If a shell, then depth is 1 + depth of next inner doll (maybe a solid, maybe also a shell)
| solid => 0                      -- *base case*
| shell d' => 1 + depth d'        -- recursive case because you're calling the depth func (when returning 1 + depth d') in the midst of defining it
...
| d => bad_depth' d       -- not a structural recursion, will not terminate. d is not a proper smaller part of the d object unlike with solid
...
| d => bad_depth d -- also an infinite loop. Calls itself on the same doll

#check (bad_depth d3)           -- Term of type Empty!!! Should never be able to return type Empty (even valid func Empty → Empty is inhabited (true)). That's why if you hover over error in func name, you see Lean rejects the func
...
inductive Nat : Type -- same format as Doll. This is the def of Nat Type in Lean.
| zero 
| succ (n' : Nat)

-- *Remember* Natural numbers are positive integers.
...
| n' + 1 => false           -- n' + 1 for (Nat.succ n')    NOTE notation: 1 + n' in 311 doesn't work, but n' + 1 works. This is just what's provided/recognized  
...
-/
def pred: Nat → Nat --the predecessor of the argument, analogous to returning inner doll of the Doll func.
| 0 => 0
| (Nat.succ n') => n' -- want predecessor of (successor of some number)--deconstruct the successor applied to that number
-- Answer here
...
| 0 => solid
| n' + 1 => shell (mk_doll n') -- we can return the doll based on the number of layers/depth.
-- Answer here

-- test cases
#check mk_doll 3
#reduce mk_doll 3 -- calls second option, which returns shell of another doll of shell of another doll...solid eventually.
...
| 0, 0 => true
| 0, n' + 1 => false -- if you don't have zero (second arg), then you must have a successor of some n', whether that's successor of 0 or a greater num. Shorthand for Nat.succ n' is n' + 1.
| n' + 1, 0 => false
| (n'+1), (m' + 1) => nat_eq n' m' -- this is true when n' and m' are equal.
...
their sum. Hint: Case analysis on the second argument.
 -/
-- either 0 + m or n' + 1 + m, which is 1 + (n' + m')
#check Nat.add
...
def l2 := List.cons 1 l1  -- [1, [[0], []]] -- mistake: notes originally said [1, [0]]
...
| [] => 0
| _::t => 1 + len t -- _ is the head, t is the tail. (Can check by hovering over _ :: t if you're unsure)

#reduce len []
#reduce len [3,2,1,0] -- 3 is the head, rest is the tail. Calling len causes recursion after the plus sign: 
--len[3,2,1,0] = 1 + len[2,1,0] 
-- = 1 + (1 + len[1,0])
-- = 1 + (1 + (1 + len[0]))
-- = 1 + (1 + (1 + (1 + len[])))
-- = 1 + (1 + (1 + (1 + (0)))) = 4
