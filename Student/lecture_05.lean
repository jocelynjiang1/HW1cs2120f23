...
-/
--return type is γ 
#check glue_funs -- shows return type is γ
/-!
...
def compose' {α β γ : Type} :
  (β → γ) →
  (α → β) → 
  (α → γ) /-this could mean return is a function, or takes a third arg, and return is γ-/
| g, f, a => g (f a)
...
def compose {α β γ : Type} (g : β → γ) (f : α → β) := λ a => g (f a) /-after the := in this func def is pattern matching-/
...
# Create a composed function h = g ∘ f
h = compose(g, f)

# Test the composed function
result = h(3)  # This should give you (3 + 1) * 2 = 8
print(result)
-/
