...
| and
| or
| imp --constructor standing for implies
...
infixr:25 " ⇒ " =>  Expr.bin_exp binary_op.imp --notation uses constructor above
...
| binary_op.and => and
| binary_op.or => or --and and or in lines 52 and 53 are using recognized/established keywords in Lean
| binary_op.imp => implies --had to define our own implies function above
...
### Validity and Unsatisfiability --valid if true in any world. Propositional variables can take values True or False
...
and false. We call such a proposition *unsatisfiable.* -- e.g. x and not x: no value for x makes this true. This proposition is unsatisfiable
...
be true as well. We call such a proposition *valid*. -- e.g. x or not x: always true, valid proposition
...
-/
-- *** satisfiable: some world where it's true/some value(s) for x can make this prop var true
--***Having a func in Lean like what we prove in our HWs shows it's valid
/-!
## EXERCISE: 
...
#eval c.n -- using line 17 and others (what was previously defined) for 198-201
#eval var.n c -- equivalent to the prev line
/-!
### Atomic Propositions

Define B, C, J and A as corresponding atomic propositions (Expr) 
-/
def B := {b} --we want B to correspond to expression form of our var b. So "b" is a var while "B" is now a var expr
-- ^ is an atomic propositional *expression*
def C:= {c} -- atomic propositional *expression* built from var c
def J:= {j}
def A:= {a}
...
-- implies: \ =>
def e0 := (¬J ∨ ¬C) ⇒ ¬ (J ∨ C)
#eval eval_expr e0 (λ v => false) --valid prop because should always be false
