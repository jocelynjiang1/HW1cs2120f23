...
let proof_of_bornotb := em B --if I accept em B (B or not B), then I can get a proof of B
...
| Or.inr na => match proof_of_bornotb with -- could also just be Or.inr na => Or.inl na (and that's it because don't need b at all to show ¬A V ¬B if na given).
  | Or.inl b => Or.inl na
  | Or.inr notb => Or.inr notb

--axiom says we'll accept the deifnition wihtout further proof. Axiom for em returns a proof of little p V /not p given P V ¬P.
--Trick to classical reasoning: apply em to a proposiiton ot get a proof. Then do case analysis on that proof/disjunction
