...
### Enumerated Types -- has a number of constant values, produced by constructors that take no args
...
product type as representing an *ordered pair* of x
...
inductive Object : Type /-"inductive" signals about to specify a type-/
| rock
| paper /-func w/ no args = a constant; has nothing coming in. Rock,paper,scissors are constructors of the Type Type (prev line), but they don't have any args-/
| scissors /-We can see that rock/paper/scissors are of Type Object b/c live inside the Object namespace-/
open Object
...
- The name we're giving to our new type is *Object*. So Object is a type. The Objects can be constructed as rock, paper, and scissors--since these three constructors take no args (allowing no more customization), Object is an enumerated type. 
...
#check rock
#check Object --Object is a Type while rock is an Object
...
inductive Result : Type /-an enumerated type-/
| wins /-constructors that take no args, and we build value by invoking one of these options by themselves-/
| loses /-This isn't pattern matching. Pipe symbol here gives value to this type w/ these 3 constructors-/
| ties /-so wins, loses, ties are all of type Result. They are themselves...something-/
...
def play : Object → Object → Result /-we play one *value of* Type Object and another value of Type Object to get a (value of) Result-/
| scissors, scissors => ties /-Result is a Type, and ties is a Result-/
| scissors, paper => wins --open Result to be able to directly reference them. If you don't, have to specify in each line "Result.ties, Result.wins,..."
| scissors, rock => loses
...
/-play the game w/in RPS namespace-/
#reduce play scissors rock --reduce is kind of another eval. Can use reduce when Lean doesn't know how to print resulting value.

end RPS 
/-Product type is like a box containing compartments that each hold an object of diff Type.-/
/-!
## Polymorphic Types
...
inductive Box (α : Type) : Type --if we say Box Nat, the type is just Type. Box is a polymorphic Type.
| put (a : α) -- we create a val of that Type w/ put. put is a constructor, probably defined in Lean
-- We give Box a Type, and it returns a Type. Box is a Type -> Type func

#check Box Nat

def box_containing_zero : Box Nat := Box.put 0 --the func will hold a Box, Box Nat, holding 0.
#check box_containing_zero -- type is Box Nat, which happens to contain a 0.

def box_containing_hello: Box String := Box.put "Hello" --type: String -> Box.String
def box_containing_hello' := Box.put "Hello" --we don't need to say that it's type Box String outright. Lean can infer because it's implied. Hello is a term of type Box String. The application term is the result. Constructor applications don't do anything.
#check box_containing_hello -- type is Box String

#check (Box) --parentheses allow us to clearly see that it's a T>T func
#check Box.put
--a constructor of a polymorphic type is itself polymorphic
--values are built using constructors
#check (@Box.put) --type is Type to value of that Type to Box containing that Type
/-!
Let's explain it.
...
--turn off implicit types w/ @ prefix
...
the box. 
-/
--pattern matching to eliminate structure around the box

def get {α : Type} : Box α → α --given a type of Box alpha, you get alpha (what's inside the box)
| (put o) => o --box must have been constructed w/ box.put. o is the name we give to what's inside box. Like undoing the put, having put o become just o, contents inside.
...
#eval get boxed_nat     -- expect 1 -- boxed_nat defined above w/ Box.put 1
#eval get boxed_bool    -- expect true -- boxed_bool defined too
#eval get boxed_string  -- expect "Hello!". To get inside a data structure, eliminate what's around the data inside you want.boxed_string
--put was the constructor to get "o" into box.
...
inductive Prod (α β : Type) : Type -- takes two args but one constructor. Won't conflict w/ any prev namespaces
| pair (a : α) (b : β) --"putting the two objects into the box"
--Prod is a Type -> Type -> Type
open Prod 
...
#check (@pair) --pair is a polymorphic func that implicitly takes two args, a value of α and β , then returns Prod α β ("ordered pair of the two objects")
...
def first {α β : Type} : Prod α β → α --first will be called to return first element, second func to return second element
| (pair a _) => a
...
def pair0 := Prod.mk true 10 --type = Prod Bool Nat
#check pair0 --elements of product type are pairs. The type is Bool cross Nat, representing the ordered pair
...
#check (@Prod.fst) --if you apply to pairs, will eliminate (fst and snd are eliminators)
#check (@Prod.snd)
...
-- Sum types vs product: w/ product type, when given a value alpha, we know product type will have the alpha, and if given beta, we know it'll at least have a beta. If given a sum type, we know contains either one type or the other (xor for sum type vs and for product type)
namespace cs2120
...
def a_sum1 : Sum Nat Bool := Sum.inl 1 -- w/o type declaration Sum Nat Bool, wouldn't work because Lean can't infer what the missing arg is--because in func def, two args expected but their types not given by default.
...
| (Sum.inl _) => "It's a Nat"  -- underscore means we don't need to be concerned about what follows the left arg (Bool) rn. In the case we have a Nat (left constructor), then we return a particular String. And same idea for Bool constructor
...
that converts any Bool to a string. Here's what
that looks like. 
-/
...
def elim_sum2 : 
  ...
  (Bool → String) → 
  String
| (Sum.inl n), n2s, _ => n2s n -- match which constructor was used to build the incoming constructor
| (Sum.inr b), _, b2s => b2s b -- here, given we don't have the Nat this time, we only look at first constructor and String returned. Deconstructs, results (after the =>) being related to the input given.
--If the func/constructor is inr, we pull out the Bool...
/-!
