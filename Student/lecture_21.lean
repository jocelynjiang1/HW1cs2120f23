...
| s ∩ t       | { a \| a ∈ s ∧ a ∈ t }     | λ a => s a ∧ t a                  | -- s intersect t is the set of all values a such that a is in s and a is in t.
| s ∪ t       | { a \| a ∈ s ∨ a ∈ t }     | λ a => s a ∨ t a                  |
| sᶜ          | { a \| a ∉ s }             | λ a => s a → False                |
| s \ t       | { a \| a ∈ s ∧ a ∉ t }     | λ a => s a ∧ (t a → False)        | -- all the elements in s that are not in t, or if you deprive s of t's values, what's left of s?
| s ⊆ t       | ∀ a, a ∈ s → a ∈ t  ...   | λ a => s a → t a ...              | -- subset relationship is transitive.
--need to prove set theory definition of s ⊂ t (∀ a, a ∈ s → a ∈ t) to prove s ⊂ t
| s ⊊ t       | ... ∧ ∃ w, w ∈ t ∧ w ∉ s  | ... ∧ ∃ w, (t w) ∧ (s w → False)  | -- proper subset: a subset, and there's some element(s) in the parent set that are not in the member set/subset. That's what makes it a proper subset.
--same set theory definition as for subset, plus there exists an element that is in t and not in s.
| 𝒫 s         | { b : Set α \| b ⊆ univ }  | λ b => b ⊆ univ                   | -- power set -- the set of all subsets of set s. If s is {1,2,3}, then power set of s is {∅, {1}, {2}, {3}, {1,2}, {1,3},{2,3},{1,2,3}} (remember a set is a subset of itself)
-- remember univ means set of all numbers. If s is set of type α, then power set is set of all sets of type alpha that...?


-- Given n elements in s, there exist 2^n subsets of s.
-- Cardinality of s = n, number of elements in s.
-- Know how to apply these operations/concepts--don't need to know in Lean but "paper and pencil"

  -- set is a predicate with 1 arg, and if you apply the predicate to the arg and resulting proposition is true, then that arg is in the set.
  -- what about two args? A binary relation... E.g. 3 < 5 is a proposition. < is like the predicate, and it takes two args.
  -- sets are predicates that take 1 arg. Binary relations are predicates that take 2 args
  -- Equality another ex of a binary relation
  -- what makes a binary relation symmetric? if a = b, then b must = a (more like "a is related to b" and "b is related to a"). Is equality a symmetric relation? Yes. E.g. is Facebook friends a symmetric relation? Yes. What about is bigger than? No.

  -- reflexive relation: every object is related to itself. Is < reflexive? No--3 is not < 3. Is <= reflexive? Yes. Is equality a reflexive relation? Is every object *equal* to itself? Yes. His other example is knows of: reflexive but not symmetric...
  -- transitive: if a is related to b by some r, and b is related to c, then if r is transitive, then a is related to c. Is Facebook friends transitive? No--can be friends with someone but not friends with their friend.
  -- < and = are both transitive. Equality is symmetric, reflexive, and transitive (called an "equivalence relation")
  -- equivalence relations divide memebrs into equivalence classes--where everyone within a class is related to everyone else within class, but no one is related between diff equiv classes.
  -- ^ This separation = "partition." Their intersections are the empty set ("disjoint"). Equivalence classes are disjoint.
  -- Exhaustive: if you take union of all equiv classes, then you get everyone

  -- More examples: is a divisor of is transitive. E.g. 2 is a divisor of 12, and 12 is a divisor of 144, so 2 is also a divisor of 144.

-/
-- Proof by induction
-- used to prove only a certain kind of prop: a universal generalization.
-- applies an induction axiom
-- Goal w/ ex: prove ∀ (n: Nat), P n (prove that for every natural number n, n has the property P)
-- p 0 -- we assume we have a proof that 0 satisfies p (the BASE CASE)
-- inductive proof: for any natural number n', if n' satisfies p, then n'+1 also satisfies p.
-- Aka ∀ n, P n' => P(n'+1)
-- if we can assume 0 has property P, then if we can assume that if any number has property P that next number up has P as well,
-- we know 0's next, 1, also has property P. And 2 has property P. And 3,4,5...
-- let P(n) = sum of (sigma) numbers 0 to n = n(n+1)/2. Does n(n+1)/2 relation hold for every natural number?
-- Prop we're trying to prove: ∀ n, P n
-- Steps:
-- prove we have P 0
-- An implication that if we have Pn, we also have P(n+1)... ∀ n, P n => P(n+1). Assume premise, show conclusion follows from it.
-- show n(n+1)/2 = (n+1)(n+1+1)/2

-- can we derive the induction axiom for natural numbers? Axiom comes from our inductive definition of natural numbers
-- induc axiom for natural numbers: ∀ (P: Nat → Prop), P 0 => ∀ (n: Nat), P n => P (n+1) => ∀ n, Pn
-- we want to conclude for ∀ n, P n.
-- "for any property of nat numbers, if 0 has that property, then any time n has that property, then n+1 property"
  -- so if you have to prove for all n, P n, then first prove it's true of 0. Then, show that if n had that property P, then the next element must have it too.
-- Nat.ind -- axiom in Lean.
-- can we do induction for other types other than Nat? Yes! E.g. type List. First show it's true for empty list, then if it's true for a list, then it's true for a list with one more element in it.
