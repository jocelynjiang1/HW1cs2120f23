...
--NOTES: a model is like an interpretation of variables that makes the proposition true. E.g. X => Y.
--A model for X and Y could be X = False, Y => True because these two valuations make the prop true.
--if there's a possibility of false, there'll be counterexample(s)

--valid = always true. Mathematical theorems are valid.
--When doing proofs, we're looking for validity, not (just) satisfiability.
--X => Y == Y => X?
--model for counterexample to^
--(X => Y) => (~X => ~Y)
--X = F, T (true) => (false)
--"no-fail strategy"/if you're stuck for finding a model: think about a truth table/plug in possible boolean vals

## Higher-Order Functions On Lists

### List.map -- turns a list to a list. Applies provided func to each element in given list, returns resulting list
...
/-!
### List.foldr -- takes a list, some binary op, applies that op to entire list. "Folds down the list" to a single value
-- e.g. if bin op is addition, takes sum of all elements in list
-- how to find sum of lengths of list's element strings? Take head's and add with tail elements' lengths
-/
...
#check @List.foldr 
-- takes a bin op, a list to be folded, and second arg is the answer to the base case
#eval List.foldr Nat.add 0 [1,2,3,4,5]  -- expect 15
-- add head, 1, to sum of the tail. What's the sum of the tail? add 2, to sum of new tail. i.e. 1 + (2 + ...)
#eval List.foldr Nat.mul 0 [1,2,3,4,5]  -- expect 120, oops! (expect 120 but gives 0 because 0 given)
#eval List.foldr Nat.mul 1 [1,2,3,4,5]  -- expect 120, ah! --CORRECT second argument
--SECOND ARG must be RELATIVE to the operator. 0 is the identity element for .add while 1 is identity elment for .mul.
-- Mistake was pasing wrong id element for .mul


/-!
### List.filter -- goes through all elements and returns sublist for which the func is true
-/
...
-/
-- "Image under a function is just the object when you apply that function"
#check @List.filter

#eval List.filter (λ (n : Nat) => n%2 == 0) [0,1,2,3,4,5,6,7]


-- Map and filter are not the same. Map always returns a list of equal size as the input. Filter's return list won't necessarily return list of equal size.
...
def eval_expr : Expr → Interp → Bool -- recursive
| Expr.true_exp,           _ => true
| Expr.false_exp,          _ => false
| (Expr.var_exp v),        i => i v
| (Expr.un_exp op e),      i => (eval_un_op op) (eval_expr e i)
| (Expr.bin_exp op e1 e2), i => (eval_bin_op op) (eval_expr e1 i) (eval_expr e2 i) -- evaluate subexpressions using eval_expr (recursion)
-- then, w/ truth vals of e1 and e2, obtain overall expr's truth val
...
--"input side of truth table is itself recursive"
...
-- Mikhail
def make_bool_lists: Nat → List (List Bool) --takes nat and gives a list of lists of bools (where truth table rows are the "lists of bools". 
-- Each row/list of bools has interpretation of each variable. Gives us "input row(s) of truth table")
| 0 => [[]]
| n + 1 =>  (List.map (fun L => false::L) (make_bool_lists n)) ++ -- where n is a number of variables, L is a list of bools. returns another list of bools with false at the start/head
            (List.map (fun L => true::L) (make_bool_lists n))

#eval make_bool_lists 0 --the base case returns 1 interpretation
...
def bool_list_to_interp : List Bool → Interp -- takes a list of bools and returns corresponding value of the bool
...
def truth_table_outputs' : Expr → List Bool --takes an expression and returns the output column, the column resulting from evaluating vars
| e =>  eval_expr_over_interps e (mk_interps_vars (num_vars e))
where eval_expr_over_interps : Expr → List Interp → List Bool
| _, [] => []
| e, h::t => eval_expr_over_interps e t ++ [eval_expr e h] --remember eval_expr evaluates expression's vars under given interpretation, h here (interp of expression under first interp in list of interps)
...
#eval List.foldr or false (truth_table_outputs (X ∧ Y)) --using foldr because want a single bool result indicating satisfiability

/-!
#### n-ary And and Or functions
-/

def reduce_or := List.foldr or false -- a func waiting for a list of bools. returns true if any of the list elements is true.
-- it's a func doing the above^ because we're using List.foldr, which is a func taking three inputs, but only two are given here.
def reduce_and := List.foldr and true
--Use reduce_or for finding satisfiability (see if any of them is true)
--Use reduce_and for finding validity
--the false and true args are the identity elements (remember from foldr above)
...
-- ^ returns Bool based on reduce_or (if any of truth table outputs is true)
def is_valid (e : Expr) : Bool := reduce_and (truth_table_outputs e)
def is_unsat (e : Expr) : Bool := not (is_sat e)
def is_unsat' (e: Expr) : Bool := is_sat ¬e --is it the same as above?
-- see if every answer is false.
-- for an expr to be unsatisfiable (false in all cases) means its *negation* is valid (true in all cases)

/-!
## Models and Counterexamples

--Not every expression has a corresponding model.
--Some expression for which there's no answer
...
| e =>
  let interps := mk_interps_expr e --given an e, we make a list of all the interps for the expr
  find_model_helper interps e --pass in our list of interps
where find_model_helper : List Interp → Expr → Option Interp --possibly return an interp. Option Interp, not just Interp, because not every Expr has a model. Parital func = not an interp for every expr. In math: e.g. 1/x
| [], _ => none --return none if there's no model
| h::t, e => if (eval_expr e h) then some h else find_model_helper t e --alternative outcome: some h, the interp that makes the expr true if we don't find an interp that returns none (note recursion)
--Option type
#check @Option --infoview says it's of type Type -> Type, so it's prob polymorphic type
-- If you look at Lean's description of Option type, shows two possibilities: its value is either some or none. Option allows you to return none if needed.
-- In this case, allows us to return none if there are no interps, which can happen since some funcs don't have models (interps that satisfy)
-- Option namespace is open by default
...
def count_models := List.length ∘ find_models --apply find_models first. Apply List.length to list returned from find_models
...
def Y' := {var.mk 1}
def Z := {var.mk 2}
#eval truth_table_outputs (X' ∧ Y') --btw, X' and Y' are defined above the same as here
...
#check ((X ⇒ Y) ⇒ (¬X ⇒ ¬Y))
#eval is_valid ((X ⇒ Y) ⇒ (¬X ⇒ ¬Y)) --evaluates to false    
...
#check ((X ⇒ Y) ⇒ (¬Y ⇒ ¬X)) --CONTRAPOSITIVE of original proposition
#eval is_valid ((X ⇒ Y) ⇒ (¬Y ⇒ ¬X))    -- says true
#reduce find_counterexamples_bool ((X ⇒ Y) ⇒ (¬Y ⇒ ¬X)) --no counterexamples, but this func doesn't seem to work right bc something commented out
...
-/
#eval count_models (X ∨ Y) --3 models/3 interps that make the prop true
